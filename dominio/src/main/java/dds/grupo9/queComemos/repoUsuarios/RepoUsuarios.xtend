...
class RepoUsuarios extends RepoDefault<Persona> {
	...
	def aceptarUsuario(Persona persona) {
		if (estaEnPendientes(persona)) {
			...
			throw new NoLoTieneException("El usuario ingresado no se encuentra en lista de pendientes")
	}
	...
	def filtrarListaPorNombreYCondiciones(Persona persona) {
		usuariosRegistrados.filter[usuario|usuario.coincidenCondiciones(persona)].filter[usuario|
			usuario.coincideNombre(persona)]
	}
	...
	def solicitarIngreso(Persona perfilUsuario) {
		if (perfilUsuario.usuarioValido)
			this.agregarAPendiente(perfilUsuario)
		else
